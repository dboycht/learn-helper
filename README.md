...
- **自检**：「答题设置」里**两个页签各有一对测试按钮**，结果都显示在窗口底部的详情框里。
  - **内部答题 API**：「测试连接」请求 `/check_version` 确认地址可达；「测试图片」把合成图发给 `/solve`。
  - **自配大模型**：「测试连接」发一句纯文本探活；「测试图片」把合成图发给 `/chat/completions`。
  - ⚠️ **「测试连接」只发纯文本，不能证明模型能读图片** —— 要验证识图必须点「测试图片」。
  - 「测试图片」在内存里合成 3 张内置测试题（单选/多选/填空，含明确期望答案），逐题报告
    **返回内容、耗时、是否识别正确**，并把「接口不通 ✗ / 接口通但答案不符或为空 ⚠ /
    识别正确 ✓」三种情况分开显示；不需要打开学习页也能判断模型是否可用、识图是否准确。
  - 图片以 base64 data URL 发送，并带 `detail: "high"`（题目截图字小且密，低分辨率容易看错）。
  - 「测试连接 / 测试图片」**直接使用输入框里当前填的值，不需要先保存**；
    若与已保存的不同，结果里会提醒你记得点「保存」（否则答题/刷课仍用旧配置）。

## 运行
1. Python 3.10；依赖：`pip install playwright requests`
   （只连接本机已安装的浏览器，无需 `playwright install` 下载内核）。
   另需 `pip install pillow` —— 仅「测试图片」自检功能用它合成测试题图片。
...
## 页面丢失防护
沙盒浏览器是可以被手动关掉的，程序对此有防护：

- 每次成功锁定学习页时会**记住它的地址**（写入 `config.json` 的 `last_page_url`）。
- 点「启动刷课」时若发现所选页面已不存在（例如你刚把浏览器关了），会**自动跳回上次记住的
  学习页**，等它加载完再继续，不会停在一个空白页上空跑。
- 若连记录都没有、或跳转失败，会**明确弹窗提示「页面已丢失」并停止**，并在进度栏留下
  「已停止：页面丢失」，而不是默默跑到"未找到下一页按钮"就结束。
- 点「检测/刷新网页」时若一个标签页都找不到，也会先尝试自动跳回再列出来。
