...
## 版本
- **1.0.2**（当前）：纯刷课（视频/文档）+ 答题。答题默认接入**内部答题 API**
  （自建后端 `POST /solve`，按卡密与设备指纹扣点，客户端零配置）；
...
- 远端调用点全部指向同一个自建后端：`/points`、`/check_version`、`/video_heartbeat`、
  `/solve`、`/solve/checkout`。

## 答题方式（面板「答题设置」→「答题方式」）
| 方式 | 说明 | 是否扣点 |
| --- | --- | --- |
| **内部答题 API**（默认） | 截图 + 题干发给自建后端 `POST /solve`，答案回填页面 | 是（卡密点数） |
| 自配大模型 | 直连你自己的 OpenAI 兼容视觉模型接口 | 否 |
| 仅识别不答题 | 只把题型/题干写进日志，便于排查平台改版 | 否 |

- 一批题目的处理顺序：**识别（截图/题型/题干）→ 并发求解 → 填涂 → 按实际填涂成功数结算扣点 → 提交/暂存**。
- 并发数、单题超时、失败重试、是否自动结算，都在「答题设置 → 内部答题 API」里配。
- 后端返回 `402`（点数不足）时立即终止流程并弹窗提示，不会继续填涂或提交。
...
2. 配置后端地址（可选，默认 `http://127.0.0.1:8000`）——环境变量 `LH_SERVER_URL`，
   ...
   ```json
   {
     ...
     "answer": { "mode": "server", "solver_timeout": 240, "workers": 4, "retry": 2, "checkout": true },
...
5. 首次做题前，先点「答题设置」→「内部答题 API」填好**后端地址**与**真实卡密**，点「测试连接」验证
   （会请求 `/points` 并回显余额）；提交方式可选「自动提交」/「仅暂存」。
...
契约（请求/响应字段、超时、错误码）见 `docs/技术文档.md` §5。
`/solve` 请求体：`card_key`、`image`(base64)、`question_type`、`num_blanks`、
`text_hash_source`、`school_id`、`device_id`；响应：`hash_id`、`answer_key`、
`text_answers`、`remaining_points`、`cached`。
