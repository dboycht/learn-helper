...
- **自检**：「答题设置 → 内部答题 API」有两个测试按钮——
  - **测试连接**：请求 `/check_version`，确认后端地址可达；
  - **测试图片**：在内存里合成 3 张内置测试题图片（单选/多选/填空，含明确期望答案）
    发给 `/solve`，逐题报告**返回内容、耗时、是否识别正确**，并把
    「接口不通 ✗ / 接口通但答案不符或为空 ⚠ / 识别正确 ✓」三种情况分开显示。
    借它可以在不打开学习页的情况下判断后端模型是否可用、识图是否准确。
